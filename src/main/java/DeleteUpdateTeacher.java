...
/**
 * Servlet implementation class DeleteUpdateTeacher
 */
@WebServlet("/DeleteUpdateTeacher")
public class DeleteUpdateTeacher extends HttpServlet {
...
String btn = request.getParameter("btn");
		...
		if(btn.equals("Update"))
		{
			try
			{
				...
				String qr = "Update teacher set name=?,degree=?,branch=?,password=?,email=?,number=? where id =?";
		...
		else 
		{
			try
			{
				...
				con.close();
				HttpSession sessiong = request.getSession();
				sessiong.setAttribute("sucess", "Delete teacher Succesfully");
			}
			catch(Exception e)
			{
